...
)
{
	...
	controller_uart_init(&(ocpp->ctrl_uart), uart, tim, OCPP_MSG_TERMINATOR);
}


Controller_Protocol_Result
controller_ocpp_update(Controller_OCPP *ocpp, Controller_TaskWrap *wrap)
{
	Controller_Protocol_Result res = controller_uart_recieve_msg(&(ocpp->ctrl_uart), ocpp->processive_buffer);
	if (res == CTRL_PTCL_RECEIVED)
	{
		res = _controller_ocpp_process_income(ocpp, wrap);
