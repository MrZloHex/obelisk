...
rss_task_1(Controller *ctrl, OCPP_MessageID t_id)
{
    ...
    OCPP_IdTag id_tag;
	int res_id = mjson_get_string(ctrl->ocpp.message.data.call.payload, strlen(ctrl->ocpp.message.data.call.payload), P_ID_TAG, id_tag, OCPP_IdTag_Len);
	if (res_id == -1)
    {
        bool reject = false;
        _controller_ocpp_make_msg(&(ctrl->ocpp), ACT_REMOTE_START_TRANSACTION, &reject, NULL, NULL);
